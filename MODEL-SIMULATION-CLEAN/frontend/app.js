...
window.SIM = {
  ...
  country:        'WLD',  // World Bank country code
  data:           null,   // last simulation result
};

/* Helper for any page that builds a /api/simulate URL — append country & speed. */
...
/* ── Country selector — injected into every sidebar's controls block ─ */
    ...
    <label>Country
      <select id="select-country"
        style="background:var(--bg-primary);color:var(--text-primary);
               ...
               padding:2px 6px;font-size:.75rem;max-width:140px">
        <option value="WLD">World</option>
      </select>
    </label>`;
  // Insert just after the "Simulation Controls" label so it sits at the top.
  const labelEl = controls.querySelector('.ctrl-label');
  if (labelEl && labelEl.nextSibling) {
    controls.insertBefore(wrap, labelEl.nextSibling);
  } else {
